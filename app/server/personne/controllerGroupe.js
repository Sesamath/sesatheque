...
module.exports = function (component) {
  component.controller('groupe', function (EntityGroupe, $groupeRepository, $personneRepository, $accessControl, $page, $form, $flashMessages) {
    ...
    const controllerGroupe = this

    /**
     * Affiche un groupe
     * @route GET /groupe/voir/:nom
     */
    controllerGroupe.get('voir/:nom', function (context) {
     ...
     */
    controllerGroupe.get('ajouter', function (context) {
      if ($accessControl.isAuthenticated(context)) {
     ...
     */
    controllerGroupe.post('ajouter', function (context) {
      ...
      try {
        ...
        // le toLowerCase() sera fait par $groupeRepository, c'est pas au controller de gérer cette logique métier
     ...
     */
    controllerGroupe.get('modifier/:nom', function (context) {
      var pid = $accessControl.getCurrentUserPid(context)
      if (pid) {
        var nom = context.arguments.nom
     ...
     */
    controllerGroupe.post('modifier/:nom', function (context) {
      try {
        var pid = $accessControl.getCurrentUserPid(context)
     ...
     */
    controllerGroupe.get('ouvert', function (context) {
     ...
     */
    controllerGroupe.get('public', function (context) {
     ...
     */
    controllerGroupe.get('perso', function (context) {
      context.layout = (context.get.layout === 'iframe') ? 'iframe' : 'page'
      var blocList = []
      flow().seq(function () {
     ...
     */
    controllerGroupe.get('quitter/:nom', function (context) {
      var nom = context.arguments.nom
     ...
     */
    controllerGroupe.get('ignorer/:nom', function (context) {
      var nom = context.arguments.nom
     ...
     */
    controllerGroupe.get('supprimer/:nom', function (context) {
      context.layout = 'page'
