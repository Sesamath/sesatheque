...
module.exports = function (component) {
  component.service('$ressourcePage', function (EntityRessource, $ressourceRepository, $personneRepository, $groupeRepository, $ressourceConverter, $accessControl, $routes, $page) {
    /**
   ...
   * }]
    ...
    $ressourcePage.getDefaultData = function (viewName) {
      ...
      data.$metas.js = [`/page.js?${version}`]
      if (viewName === 'display' || viewName === 'preview' || viewName === 'formEdit') {
        data.$metas.js.push(`/display.js?${version}`)
        if (viewName === 'formEdit') data.$metas.js.push(`/edit.js?${version}`)
      }
    ...
    $ressourcePage.printSearchForm = function (context, errorMessages) {
      ...
      getFormViewData(context, null, fakeRessource, function (error, formData) {
        ...
        fd.titre.rmq = 'Vous pouvez utiliser le caractère % comme joker, par exemple "%Pythagore%" pour trouver toutes les ressources dont le titre contient le mot "Pythagore" (il n’y a pas de distinction minuscule/majuscule)'
