...
function setCurrent(editor) {
  ...
  current = editor
  dom.log("On est passé en " +current)
}
...
try {
    ...
    try {
      ...
      multiEditor.toSimple(function (error) {
        if (error) {
          ...
          //}
          if (contenuSsTxt.indexOf(" ") === -1 || window.confirm("Attention, l'éditeur d'équation supprime les espaces (hors commande \text{})," +
  ...
  multiEditor.toSimple = function (next) {
    ...
    try {
      ...
      if (current === "simple") {
        dom.log("On était déjà en simple, on fait rien")
        ...
        //dom.log("avant de simplifier l'éditeur on récupère le texte " +contenu)
        //$textarea.empty()
        //$textarea.val(contenu)
      } else {
  ...
  multiEditor.init = function (textarea, config, next) {
    $(function () {
      dom.log("multiEditor.init avec la config", config)
      if (typeof next !== "function") next = errorCallbackDefault
      if (!config) config = {}
