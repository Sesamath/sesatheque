...
function addButton(parent, button) {
  ...
  dom.log("Ajout bouton " +button, btn)
  var value = mqExpr[button]
  if (typeof value === "string") {
    // une seule commande
    btn.addEventListener('click', function () {
      dom.log("clic sur " +button)
    ...
    // inconnu
    dom.log.error(button +" n'est pas un bouton connu")
  }
}
    ...
    try {
      $(function () {
        ...
        if (textarea.nodeName === "TEXTAREA") {
          ...
          dom.log("init mathquill avec un contenu dans un tag " + textarea.nodeName)
          $mqDiv = $(textarea)
        }
