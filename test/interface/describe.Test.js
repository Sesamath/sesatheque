...
describe('get public by oid', function () {
  ...
  it("récupère la page attendue", function (done) {
    ...
    browser.assert.text('h2', "Droites visiblement parallèles")
    done()
  })
})
...
describe('get public by origine', function () {
  ...
  it("récupère la page attendue", function (done) {
    ...
    //logInfo(browser.tabs[0]._response)
    browser.assert.success()
    browser.assert.elements('h2', { atLeast: 1, atMost: 1 })
    assert.equal(browser.text('h2').toLowerCase(), 'exercices interactifs')
    done()
  })
})